...
def extract_with_fallback(agent_dir: Path, output: Path) -> bool:
    ...
    Most native-log conversions live in the vendored Harbor adapter backend.
    This fallback keeps the CLI useful for agents that already wrote ATIF, or
    for directories where Harbor previously wrote trajectory.json.
    """
    candidates = [
